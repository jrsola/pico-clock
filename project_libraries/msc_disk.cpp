...
#define MSC_FLASH_OFFSET  DISK_FLASH_OFFSET
#define MSC_FLASH_SIZE    DISK_SIZE_BYTES
...
bool tud_msc_start_stop_cb(uint8_t lun,
                              ...
                              uint32_t bufsize)
    {
        ...
        if (start + bufsize > MSC_FLASH_SIZE)
        {
            return -1;
        }

        const uint8_t *flash =
            (const uint8_t *)(XIP_BASE + MSC_FLASH_OFFSET + start);
...
int32_t tud_msc_write10_cb(uint8_t lun,
    ...
    uint32_t write_addr = MSC_FLASH_OFFSET + start;
    ...
    // indica si la unitat és d'escriptura
